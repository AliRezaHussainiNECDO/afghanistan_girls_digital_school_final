...
/// سرعتِ پخشِ صدای «معلم هوشمند» (خواندنِ درس + پاسخ‌های گفتاری) — طبقِ
/// بازخوردِ کاربران («صدا خیلی آهسته و خسته‌کننده است»)، یک‌ذره سریع‌تر از
/// سرعتِ طبیعیِ ۱ (پیش‌فرضِ audioplayers) پخش می‌شود. این مقدار در یک‌جا
/// تعریف شده تا هر سه محلِ پخشِ صدای معلم (`LessonDetailScreen`،
/// `AiVoiceAskSheet`، `AiTeacherScreen`) دقیقاً همان یک سرعت را داشته باشند؛
/// اگر بعداً لازم شد این عدد تنظیم‌پذیر (مثلاً در تنظیمات کاربر) شود، همین‌جا
/// تغییرش کافی است. صدای پیام‌های صوتیِ ضبط‌شدهٔ واقعی (کارخانگی/چت) عمداً از
/// این سرعت پیروی نمی‌کند — آن‌ها صدای طبیعیِ خودِ شاگرد/معلم‌اند، نه TTS.
const double kAiVoicePlaybackRate = 1.2;
