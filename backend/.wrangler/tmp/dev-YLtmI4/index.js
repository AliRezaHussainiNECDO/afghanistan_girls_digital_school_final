...
__name(strToB64Url, "strToB64Url");
...
async function signJwt(payload, secret, expiresInSec = 900) {
...
__name(verifyBearer, "verifyBearer");

// src/routes/auth.ts
var ACCESS_TTL = 60 * 15;
var REFRESH_TTL = 60 * 60 * 24 * 30;
var auth = new Hono2();
var uid = /* @__PURE__ */ __name(() => crypto.randomUUID(), "uid");
function fail(code, messageFa, messageEn) {
...
async function issueTokens(db, secret, u) {
  const jti = uid();
  const expiresAt = new Date(Date.now() + REFRESH_TTL * 1e3).toISOString();
  await db.prepare("INSERT INTO refresh_tokens (id, user_id, expires_at) VALUES (?, ?, ?)").bind(jti, u.id, expiresAt).run();
...
auth.post("/register", async (c) => {
  ...
  if (needsInvite) {
    ...
    inviteRow = await c.env.DB.prepare(
      "SELECT id FROM invite_codes WHERE code = ? AND type = ? AND status = 'unused' AND (expires_at IS NULL OR expires_at > datetime('now'))"
    ).bind(code, inviteType).first();
    if (!inviteRow) {
  ...
  const currentGrade = role === "student" && body.currentGrade != null ? Number(body.currentGrade) : null;
  const awaitingParentLink = role === "parent" ? 1 : 0;
  const statements = [
    c.env.DB.prepare(
      "INSERT INTO users (id, email, password_hash, first_name, last_name, phone, role, status, current_grade, province, date_of_birth, preferred_language, awaiting_parent_link, specialty, bio) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?, ?, ?, ?)"
    ).bind(
      id,
      email,
      passwordHash,
...
var uid2 = /* @__PURE__ */ __name(() => crypto.randomUUID(), "uid");
var bannedWords = ["\u0641\u062D\u0634", "\u0628\u062F", "\u0627\u062D\u0645\u0642", "\u0644\u0639\u0646\u062A\u06CC"];
var dmIdFor = /* @__PURE__ */ __name((a, b) => {
  const ids = [a, b].sort();
  return `dm_${ids[0]}_${ids[1]}`;
}, "dmIdFor");
app.post("/conversations", async (c) => {
  const body = await c.req.json();
  const id = body.type === "admin" ? `admin_${body.participants.find((p) => p.id !== "admin")?.id ?? uid2()}` : dmIdFor(body.participants[0].id, body.participants[1].id);
  ...
  if (!existing) {
    ...
    ).bind(id, body.type, body.classId, body.className, JSON.stringify(body.participants), "").run();
  }
  return c.json({ id });
});
...
app.post("/conversations/:id/messages", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    body.senderId,
    body.senderName ?? "",
    body.senderClassName ?? "",
    body.text,
    "text",
    flagged ? 1 : 0,
    flagged ? "pending" : "none"
  ).run();
  await c.env.DB.prepare(
    'UPDATE conversations SET last_message = ?, last_message_at = datetime("now") WHERE id = ?'
  ).bind(flagged ? "\u062F\u0631 \u0627\u0646\u062A\u0638\u0627\u0631 \u0628\u0627\u0632\u0628\u06CC\u0646\u06CC \u0645\u062F\u06CC\u0631..." : body.text, conversationId).run();
  return c.json({ id, flagged });
});
app.post("/conversations/:id/messages/voice", async (c) => {
  const conversationId = c.req.param("id");
  const senderId = c.req.header("X-Sender-Id") ?? "unknown";
  ...
  const audioKey = `voice/${conversationId}/${uid2()}.m4a`;
  ...
  ).bind(id, conversationId, senderId, senderName, senderClassName, "voice", audioKey, durationMs).run();
  await c.env.DB.prepare(
    'UPDATE conversations SET last_message = ?, last_message_at = datetime("now") WHERE id = ?'
  ).bind("\u{1F399} \u067E\u06CC\u0627\u0645 \u0635\u0648\u062A\u06CC", conversationId).run();
  return c.json({ id, audioUrl: `/files/${audioKey}` });
});
app.post("/messages/:id/report", async (c) => {
  const messageId = c.req.param("id");
  const body = await c.req.json();
  await c.env.DB.prepare(
    "INSERT INTO chat_reports (id, message_id, reason, reported_by_id, reported_by_name) VALUES (?, ?, ?, ?, ?)"
  ).bind(uid2(), messageId, body.reason, body.reportedById, body.reportedByName ?? "").run();
  return c.json({ ok: true });
});
app.get("/admin/chat/overview", async (c) => {
  if (!isAdmin(c.req.header("Authorization"), c.env)) return c.json({ error: "forbidden" }, 403);
  const { results } = await c.env.DB.prepare(
...
app.get("/admin/chat/classes/:classId/conversations", async (c) => {
  if (!isAdmin(c.req.header("Authorization"), c.env)) return c.json({ error: "forbidden" }, 403);
  const { results } = await c.env.DB.prepare(
    "SELECT * FROM conversations WHERE class_id = ? ORDER BY last_message_at DESC"
  ).bind(c.req.param("classId")).all();
  return c.json(results);
});
app.get("/admin/chat/inbox", async (c) => {
  if (!isAdmin(c.req.header("Authorization"), c.env)) return c.json({ error: "forbidden" }, 403);
  const { results } = await c.env.DB.prepare(
    "SELECT * FROM conversations WHERE type = 'admin' ORDER BY last_message_at DESC"
  ).all();
  return c.json(results);
});
...
app.post("/admin/messages/:id/review", async (c) => {
  if (!isAdmin(c.req.header("Authorization"), c.env)) return c.json({ error: "forbidden" }, 403);
  const body = await c.req.json();
  await c.env.DB.prepare("UPDATE messages SET review_status = ? WHERE id = ?").bind(body.approve ? "approved" : "rejected", c.req.param("id")).run();
  return c.json({ ok: true });
});
app.post("/admin/conversations/:id/reply", async (c) => {
  if (!isAdmin(c.req.header("Authorization"), c.env)) return c.json({ error: "forbidden" }, 403);
  const conversationId = c.req.param("id");
  const body = await c.req.json();
  const id = uid2();
  ...
  ).bind(id, conversationId, body.text).run();
  await c.env.DB.prepare(
    'UPDATE conversations SET last_message = ?, last_message_at = datetime("now") WHERE id = ?'
  ).bind(body.text, conversationId).run();
  return c.json({ id });
});
...
app.get("/books", async (c) => {
  ...
  const stmt = subjectId ? c.env.DB.prepare("SELECT id, subject_id, title, page_count, uploaded_at FROM curriculum_books WHERE subject_id = ? ORDER BY uploaded_at DESC").bind(subjectId) : c.env.DB.prepare("SELECT id, subject_id, title, page_count, uploaded_at FROM curriculum_books ORDER BY uploaded_at DESC");
...
app.post("/books", async (c) => {
  ...
  await c.env.BUCKET.put(pdfKey, await file.arrayBuffer(), {
    httpMetadata: { contentType: "application/pdf" }
  });
  await c.env.DB.prepare(
    "INSERT INTO curriculum_books (id, subject_id, title, page_count, pdf_key, extracted_text) VALUES (?, ?, ?, ?, ?, ?)"
  ).bind(id, subjectId, title, pageCount, pdfKey, extractedText).run();
  return c.json({ id });
});
app.delete("/books/:id", async (c) => {
  if (!isAdmin(c.req.header("Authorization"), c.env)) return c.json({ error: "forbidden" }, 403);
  const id = c.req.param("id");
  const row = await c.env.DB.prepare("SELECT pdf_key FROM curriculum_books WHERE id = ?").bind(id).first();
...
app.post("/consents", async (c) => {
  const body = await c.req.json();
  await c.env.DB.prepare("INSERT INTO consents (id, user_id, version) VALUES (?, ?, ?)").bind(uid2(), body.userId, body.version).run();
  return c.json({ ok: true });
});
function isAdmin(authHeader, env) {
  return true;
}
...
// .wrangler/tmp/bundle-xJa9yT/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-xJa9yT/middleware-loader.entry.ts
